...
def get_user_tweets(twitter_handle): 
	handle = "twitter_{}".format(twitter_handle)
	if handle in CACHE_DICTION: 
		return CACHE_DICTION[handle]
	else: 
		result = api.user_timeline(twitter_handle)
		CACHE_DICTION[handle] = result 
		twitter_cache = open(CACHE_FNAME, 'w')
		twitter_cache.write(json.dumps(CACHE_DICTION, indent=2))
		twitter_cache.close()
		return CACHE_DICTION[handle]	
...
table_spec += 'Tweets (tweet_id PRIMARY KEY, '
...
for x in umsi_tweets: 
	tweet_tup = (x['id'], x['user']['screen_name'], x['created_at'], x['text'], x['retweet_count'])
...
time_stamp = 'SELECT time_posted FROM Tweets'
cur.execute(time_stamp)
...
retweet = 'SELECT * FROM Tweets WHERE retweets > 2'
cur.execute(retweet)
...
other_retweet = "SELECT tweet_text FROM Tweets WHERE instr(tweet_text, 'RT')"
cur.execute(other_retweet)
